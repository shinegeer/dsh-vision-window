...
/**
 * Encode an image for the vision upload path: JPEG keeps the wire payload
 * small (vision models do not need lossless input), while local pixel tools
 * keep using the original bytes. Falls back to the input bytes when sharp is
 * unavailable or when JPEG would not actually shrink the payload.
 */
export async function jpegForVision(bytes, quality = 82) {
  if (extOf(bytes) === 'jpg') return bytes;
  try {
    const mod = await loadSharp();
    const sharp = mod.default || mod;
    const out = await sharp(bytes, { failOn: 'none' })
      ...
      .toBuffer();
    if (!out || out.length === 0) return bytes;
