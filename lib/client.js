window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			".ui-paste-image-float{position:fixed;z-index:200;box-sizing:border-box;width:260px;border:1px solid var(--dsw-alias-border-inverted);background:var(--dsw-specific-menu);box-shadow:var(--dsw-shadow-lv3);border-radius:14px;padding:10px;display:flex;flex-direction:column;gap:8px;color:var(--dsw-alias-label-primary);font-size:13px;line-height:20px}",
			...
			".ui-paste-image-float-actions{display:flex;gap:8px}",
			".ui-paste-image-float-pick{height:28px;flex:1;border:1px solid var(--dsw-alias-border-l2);border-radius:14px;background:transparent;color:var(--dsw-alias-label-secondary);font-family:inherit;font-size:13px;cursor:pointer}",
			".ui-paste-image-float-pick:hover{background:var(--dsw-alias-interactive-bg-hover)}",
			".ui-paste-image-float-go{height:28px;flex:1;border:none;border-radius:14px;background:var(--dsw-alias-interactive-bg-primary);color:var(--dsw-alias-label-inverse);font-family:inherit;font-size:13px;cursor:pointer}",
			".ui-paste-image-float-go:disabled{opacity:.5;cursor:default}",
			...
			".ui-paste-image-cfg{border-top:1px solid var(--dsw-alias-border-l2);padding-top:8px;display:flex;flex-direction:column;gap:6px}",
			...
			".ui-paste-image-cfg-status{font-size:12px;line-height:16px;color:var(--dsw-alias-label-tertiary)}",
		...
		var STR = {
			zh: {
				...
				needBaseAndModel: "地址和模型名不能为空",
				needBaseAndModelForTest: "请先填写地址和模型名",
				needKey: "请填写密钥",
				testing: "正在测试…",
				connected: "连接成功",
				saveFailed: "保存失败",
			...
			en: {
				...
				cfgTitle: "Configure the vision model (base URL / key / model)",
				close: "Close",
				...
				baseUrl: "Base URL",
				apiKey: "API key",
				keySet: "Set (leave blank to keep)",
				model: "Model",
				apiType: "API type",
				...
				needBaseAndModel: "Base URL and model are required",
				needBaseAndModelForTest: "Fill in the base URL and model first",
				needKey: "Fill in the API key",
				testing: "Testing…",
				connected: "Connected",
				...
				failed: "Failed",
				resultHeader: "[Image recognition result]"
			}
		};
		...
		function PasteImageButton(props) {
			...
			var cfgState = React.useState({ baseUrl: "", model: "", apiType: "chat", maxTokens: 0, language: "zh" });
			var cfg = cfgState[0];
			var setCfg = cfgState[1];
			var cfgHasKeyState = React.useState(false);
			var cfgHasKey = cfgHasKeyState[0];
			var setCfgHasKey = cfgHasKeyState[1];
				...
				rpc.call(CHANNEL, "get-config", {}).then(function (res) {
					if (res && res.ok === true && res.value) {
						setCfg({
							...
							apiType: res.value.apiType || "chat",
							maxTokens: res.value.maxTokens || 0,
							language: res.value.language === "en" ? "en" : "zh"
						});
						setCfgHasKey(!!res.value.hasKey);
			...
			function onToggleConfig() {
				if (!configOpen) { setKeyInput(""); setCfgStatus(null); setConfigOpen(true); loadConfig(); }
				else setConfigOpen(false);
			}

			function onSaveConfig() {
				if (!cfg.baseUrl.trim() || !cfg.model.trim()) { setCfgStatus({ kind: "error", text: t("needBaseAndModel") }); return; }
				setCfgBusy(true);
				setCfgStatus(null);
				rpc.call(CHANNEL, "set-config", { baseUrl: cfg.baseUrl.trim(), model: cfg.model.trim(), apiKey: keyInput, apiType: cfg.apiType, maxTokens: cfg.maxTokens, language: cfg.language }).then(function (res) {
					setCfgBusy(false);
					if (res && res.ok === true && res.value) {
						setCfgHasKey(!!res.value.hasKey);
						setKeyInput("");
			...
			function onTestConfig() {
				if (!cfg.baseUrl.trim() || !cfg.model.trim()) { setCfgStatus({ kind: "error", text: t("needBaseAndModelForTest") }); return; }
				if (!keyInput.trim() && !cfgHasKey) { setCfgStatus({ kind: "error", text: t("needKey") }); return; }
				setCfgBusy(true);
				setCfgStatus({ kind: "info", text: t("testing") });
				rpc.call(CHANNEL, "test-vision", { baseUrl: cfg.baseUrl.trim(), model: cfg.model.trim(), apiKey: keyInput, apiType: cfg.apiType, maxTokens: cfg.maxTokens, language: cfg.language }).then(function (res) {
					setCfgBusy(false);
					if (res && res.ok === true && res.value && res.value.ok === true) {
						setCfgStatus({ kind: "ok", text: t("connected") });
					} else {
						var msg = (res && res.error && res.error.message) || (res && res.value && res.value.message) || t("testFailed");
			...
			function onClearConfig() {
				setCfgBusy(true);
				setCfgStatus(null);
				rpc.call(CHANNEL, "set-config", { baseUrl: "", model: "", apiType: "chat", maxTokens: 0, language: cfg.language, clearKey: true }).then(function (res) {
					setCfgBusy(false);
					if (res && res.ok === true) {
						setCfg({ baseUrl: "", model: "", apiType: "chat", maxTokens: 0, language: cfg.language });
						setCfgHasKey(false);
						setKeyInput("");
			...
			function measureComposer() {
				...
				return { left: Math.max(0, rect.left - 260 - 12), bottom: Math.max(0, vh - rect.bottom) };
			}
			function onToggle() {
				if (!open) {
			...
			if (configOpen) {
				configEl = React.createElement("div", { key: "cfg", className: "ui-paste-image-cfg" }, [
					cfgRow("baseUrl", t("baseUrl"), cfgTextInput({ value: cfg.baseUrl, placeholder: "https://api.example.com/v1", onChange: function (e) { patchCfg({ baseUrl: e.target.value }); } })),
					cfgRow("key", t("apiKey"), React.createElement("input", { type: "password", className: "ui-paste-image-cfg-input", value: keyInput, placeholder: cfgHasKey ? t("keySet") : "sk-…", autoComplete: "off", onChange: function (e) { setKeyInput(e.target.value); } })),
					cfgRow("model", t("model"), cfgTextInput({ value: cfg.model, placeholder: "gpt-4o-mini / qwen-vl-max …", onChange: function (e) { patchCfg({ model: e.target.value }); } })),
					cfgRow("apiType", t("apiType"), React.createElement("select", { className: "ui-paste-image-cfg-input", value: cfg.apiType, onChange: function (e) { patchCfg({ apiType: e.target.value }); } }, [
					...
					cfgRow("maxTokens", t("maxTokens"), cfgTextInput({ type: "number", value: cfg.maxTokens > 0 ? String(cfg.maxTokens) : "", placeholder: t("maxTokensPlaceholder"), min: "1", onChange: function (e) { var raw = e.target.value; if (raw === "") { patchCfg({ maxTokens: 0 }); return; } var v = parseInt(raw, 10); patchCfg({ maxTokens: Number.isFinite(v) && v > 0 ? v : 0 }); } })),
