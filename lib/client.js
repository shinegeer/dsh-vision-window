window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			".ui-paste-image-float-actions{display:flex;flex-direction:column;gap:8px}",
			".ui-paste-image-float-pick{height:36px;width:100%;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;background:transparent;color:var(--dsw-alias-label-secondary);font-family:inherit;font-size:13px;line-height:20px;cursor:pointer}",
			...
			".ui-paste-image-float-go{height:52px;width:100%;display:inline-flex;align-items:center;justify-content:center;gap:8px;border:none;border-radius:14px;background:#3D5AF8;color:#ffffff;font-family:inherit;font-size:16px;font-weight:700;line-height:22px;cursor:pointer;box-shadow:0 4px 14px rgba(61,90,248,.38);transition:background .15s ease,box-shadow .15s ease,transform .05s ease}",
			".ui-paste-image-float-go svg{width:18px;height:18px;flex:none}",
			".ui-paste-image-float-go:hover:not(:disabled){background:#3551E8;box-shadow:0 6px 18px rgba(53,81,232,.46)}",
			".ui-paste-image-float-go:active:not(:disabled){background:#2F49D8;transform:translateY(1px)}",
			".ui-paste-image-float-go:focus-visible{outline:2px solid var(--dsw-alias-border-l3);outline-offset:2px}",
			".ui-paste-image-float-go[data-ready=true]{box-shadow:0 0 0 4px rgba(61,90,248,.28),0 8px 22px rgba(61,90,248,.55)}",
			".ui-paste-image-float-go:disabled{opacity:.45;cursor:default;box-shadow:none}",
